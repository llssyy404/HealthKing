...
namespace RenderHeads.Media.AVProVideo
{
	public class AudioOutputManager
	{
		...
		public void RequestAudio(AudioOutput _outputComponent, MediaPlayer mediaPlayer, float[] data, int channelMask, int totalChannels, AudioOutput.AudioOutputMode audioOutputMode)
		{
			...
			//hardcoding to 2 for now since the plugin only deals with 2 channels, will probably need to change later to cater for 
			//different numbers
			int channels = 2;
			...
			else if(audioOutputMode == AudioOutput.AudioOutputMode.Single)
			{
				...
				if(desiredChannel > 0 && desiredChannel <= channels)
				{
					for (int i = 0; i < samples; ++i)
					{
						for (int j = 0; j < totalChannels; ++j)
						{
							data[requestedPos + j] = _pcmData[mediaPlayer][storedPos + (desiredChannel - 1)];
						}
